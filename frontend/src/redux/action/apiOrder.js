...
export const startStripeCheckOut = (order,userInfo) => {
  return async (dispatch) => {
    try {
      ...
      order["customerID"] = userInfo._id; 

      const config = {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${userToken}`, // Updated key to 'Authorization'
        },
        body: JSON.stringify(order)
      };

      const response = await fetch('http://localhost:4000/stripe/create-checkout-session', config);
